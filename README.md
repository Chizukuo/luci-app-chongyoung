...
- **v2.1.3 (当前推荐)**：适配 school_hbct 新门户，具备完整的交换机防刷保护、会话保活、全架构网卡自适应、无 RTC 开机时钟防死锁与秒级自愈能力。
...
- 跨架构网络接口自适应：通过 `get_wan_device` 动态解析三层/物理网卡名，彻底解决 x86、树莓派及非 DSA 架构下 `dev wan` 报错导致的二层 ARP 与缺省路由失效问题。
- 状态闭环清理：服务退出与停止（`cleanup`）时同步清理 `/tmp/feiyoung_status`，确保 Web 状态即时显示“未运行”。
- 射频保护：休眠恢复时过滤已禁用的无线设备，防止意外开启用户手动关闭的 5G 射频。
- 协议兼容性增强：重定向探测改用不区分大小写匹配 `userip=`，密码长度校验放宽至至少 6 位，登录成功后立即校准保活定时器。

**v2.1.2** (2026-09-06) - 修复无 RTC 设备开机误判休眠死锁
- 修复无 RTC 硬件时钟的路由器开机时，因 `sysfixtime` 还原历史时间戳误判进入休眠断网的严重漏洞（强制要求必须先通过 NTP/HTTP 完成时钟授时，才允许激活计划休眠，彻底消除断网死锁）。
